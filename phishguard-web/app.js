...
function startImeiTelemetry() {
  ...
  if (telemetryWs) {
    ...
    logArea.innerHTML += `<div>[${new Date().toLocaleTimeString('en-IN')}] Disconnected from C2 WebSocket.</div>`;
    return;
  }
  ...
  logArea.innerHTML = `<div>[${new Date().toLocaleTimeString('en-IN')}] Connecting to C2 Server (ws://127.0.0.1:8080)...</div>`;

  // Connect to the real C2 WebSocket Server running on localhost
  telemetryWs = new WebSocket('ws://127.0.0.1:8080');

  if (!trackingActive) reportStolen();
  if (trackingTimer) { clearInterval(trackingTimer); trackingTimer = null; }

  telemetryWs.onopen = () => {
    logArea.innerHTML += `<div>[${new Date().toLocaleTimeString('en-IN')}] Connected! Subscribing to IMEI: ${imei.slice(0, 4)}***********</div>`;
    logArea.scrollTop = logArea.scrollHeight;

    // Subscription request
    telemetryWs.send(JSON.stringify({ action: 'subscribe', imei: imei }));
  };
  ...
  telemetryWs.onmessage = (evt) => {
    try {
      ...
      if (data.status === 'subscribed') {
        logArea.innerHTML += `<div style="color:yellow">[${new Date().toLocaleTimeString('en-IN')}] Subscription verified. Awaiting UDP datagrams...</div>`;
      } else if (data.type === 'telemetry') {
        updateMapPosition(data.lat, data.lng);
        logArea.innerHTML += `<div>[${new Date().toLocaleTimeString('en-IN')}] [UDP RX] SEQ: ${seq++} | LAT: ${data.lat.toFixed(6)} | LNG: ${data.lng.toFixed(6)} | BAT: ${data.battery}% | CID: ${data.cellId}</div>`;
        logArea.scrollTop = logArea.scrollHeight;
      }
    } catch (e) { }
  };
  ...
  telemetryWs.onerror = () => {
    logArea.innerHTML += `<div style="color:#ef4444">[${new Date().toLocaleTimeString('en-IN')}] Error: Could not connect to C2 WebSocket! Ensure node server is running.</div>`;
    logArea.scrollTop = logArea.scrollHeight;
  };
