...
function startImeiTelemetry() {
  ...
  if (telemetryInterval) {
    ...
    logArea.innerHTML += `<div>[${new Date().toLocaleTimeString('en-IN')}] Disconnected from AntiGravity UDP socket.</div>`;
    return;
  }
  ...
  logArea.innerHTML = `<div>[${new Date().toLocaleTimeString('en-IN')}] Connecting to UDP Socket over port 8883...</div>`;

  let baseLat = currentLat;
  let baseLng = currentLng;
  let seq = 1;

  setTimeout(() => {
    logArea.innerHTML += `<div>[${new Date().toLocaleTimeString('en-IN')}] Connection established for IMEI: ${imei.slice(0, 4)}***********</div>`;
    ...
    telemetryInterval = setInterval(() => {
      ...
      updateMapPosition(parseFloat(lat), parseFloat(lng));

      logArea.innerHTML += `<div>[${new Date().toLocaleTimeString('en-IN')}] [UDP RX] SEQ: ${seq++} | LAT: ${lat} | LNG: ${lng} | BAT: ${Math.floor(Math.random() * 20) + 40}%</div>`;
      logArea.scrollTop = logArea.scrollHeight;
    }, 2000);
  }, 1000);
}
