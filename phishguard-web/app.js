...
function connectC2WebSocket() {
  ...
  globalWs.onopen = () => {
    if (c2Dot) c2Dot.style.background = '#4ade80';
    if (c2Text) c2Text.textContent = 'C2 Connected';
  ...
  globalWs.onmessage = (evt) => {
    try {
      ...
      if (msg.type === 'INIT') {
        const { devices, stats } = msg.data;
        if (devices) devices.forEach(d => c2Devices.set(d.imei, d));
        if (stats) {
          scanCount = stats.scansToday || 0;
          blockedCount = stats.threatsBlocked || 0;
          setScanCounts();
        }
      } else if (msg.type === 'TELEMETRY' || msg.type === 'LOCATION_UPDATE' || msg.type === 'telemetry') {
        const data = msg.data || msg;
        if (data.imei) c2Devices.set(data.imei, data);

        const trackedImei = document.getElementById('telemetry-imei')?.value.trim();
        if (trackingActive && trackedImei && data.imei === trackedImei) {
          updateMapPosition(data.lat, data.lng);
          const logArea = document.getElementById('telemetry-log');
          if (logArea) {
            logArea.innerHTML += `<div>[${new Date().toLocaleTimeString('en-IN')}] [WS RX] LAT: ${data.lat.toFixed(6)} | LNG: ${data.lng.toFixed(6)} | BAT: ${data.battery}%</div>`;
            logArea.scrollTop = logArea.scrollHeight;
          }
          const batDisplay = document.getElementById('battery-display');
          if (batDisplay && data.battery != null) {
            batDisplay.textContent = Math.round(data.battery) + '%';
          }
        }
      }
    } catch (e) { }
  ...
  globalWs.onclose = () => {
    if (c2Dot) c2Dot.style.background = '#fbbf24'; // Yellow
    if (c2Text) c2Text.textContent = 'Reconnecting...';

    setTimeout(connectC2WebSocket, reconnectDelay);
    reconnectDelay = Math.min(reconnectDelay * 2, MAX_RECONNECT_DELAY);
  };

  globalWs.onerror = () => { /* handled by onclose */ };
}
...
function startImeiTelemetry() {
  ...
  if (globalWs && globalWs.readyState === WebSocket.OPEN) {
    ...
    if (c2Devices.has(imei)) {
      ...
      document.getElementById('battery-display').textContent = Math.round(initData.battery) + '%';
    }
  } else {
